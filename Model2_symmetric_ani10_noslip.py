...
particleViscosity1s = swarm.add_variable ( dataType="double", count=1 )
particleViscosity2s = swarm.add_variable ( dataType="double", count=1 )
...
particleVgradient = swarm.add_variable( dataType="double", count=9 )

particleStress = swarm.add_variable( dataType="double", count=6 )
...
'''
for j in mesh.specialSets["MinI_VertexSet"]:
    loc = mesh.data[j,1]
    if loc<=iceHeight:
        velocityField.data[j][0] = meshVx
'''        
for k in mesh.specialSets["MaxI_VertexSet"]:
    z = mesh.data[k,2]
    loc = mesh.data[k,1]
    if z2<z<z3 and loc>bed:
...
while step < maxSteps:
    ...
    if not step%stepsize: # if multiple of ..
        ...
        #viscosityFn1s = viscosityFn*3.1536e7 #pa*s
        #particleViscosity1s.data[:] = viscosityFn1s.evaluate(swarm)
        #viscosityFn3s = viscosityFn3*3.1536e7 #pa*s
        #particleViscosity2s.data[:] = viscosityFn3s.evaluate(swarm)
        particleShearstress.data[:] = shearStressFn.evaluate(swarm)
        particleStress.data[:] = devStressFn.evaluate(swarm)
        particleVelocity.data[:]  = velocityField.evaluate(swarm)
        Vgradient= velocityField.fn_gradient
        particleVgradient.data[:] = Vgradient.evaluate(swarm)
        ...
        xdmf_particleVgradient = particleVgradient.save('particleVgradient_' + str(step) + '.h5')
        particleVgradient.xdmf('particleVgradient_' + str(step) + '.xdmf', xdmf_particleVgradient, 
                                "particleVgradient", xdmf_swarm, "Swarm", modeltime=step)
        ...
        xdmf_particleStress = particleStress.save('particleStress_' + str(step) + '_'+ str(t) + '.h5')
        particleStress.xdmf('particleStress_' + str(step) + '.xdmf', xdmf_particleStress, 
                                "particleStress", xdmf_swarm, "Swarm", modeltime=step)
